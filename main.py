...
from database import create_viewed_users_table, insert_viewed_user_data, select_viewed_user
...
class VKBotСhat:
    ...
    def __init__(self, token):
        ...
        # для описания состояний и переходов между ними используется библиотека transitions
          self.machine = Machine(model=self, states=states, transitions=transitions, initial="START", queued=True)
          self._event = None
          self.user_search_preference = {}
          self.profiles_to_send = {}

    def create_db(self):
        # для transitions; создаем БД, если ее нет
    ...
    def filter_unviewed_users(self, profiles, user_id):
        profiles_to_send = []
        for profile in profiles:
            if select_viewed_user(user_id, profile['id']) is None:  # проверяем нет ли повторений
    ...
    def get_suggested_candidates(self, event):
        ...
        filtered_profiles = self.filter_unviewed_users(profiles, user_id)
    ...
    def view_next_profile(self, event):
        ...
        if self.profiles_to_send[user_id]:
            ...
            insert_viewed_user_data(user_id, profile['id'])
        ...
        match self.state:
            case 'START':
                if self.is_hello_message_valid():
                    ...
                    self.send_message(
                        event,
                        text="Здравствуйте! Кто вам нужен? Мужчина или женщина? (напиши М или Ж)",
            ...
            case "SET_GENDER":
                if self.is_gender_valid():
                    ...
                    if message == "М":
                        message = "Прекрасно! Напиши от какого возраста мужчину ты ищешь."
                    else:
                        message = "Прекрасно! Напиши от какого возраста женщину ты ищешь."
            ...
            case "SET_MAX_AGE":
                if self.is_age_valid(type='max'):
                    ...
                    self.send_message(event, "Введи город")
                else:
                    error_msg = self.get_invalid_input_message()
                    self.send_message(event, text=error_msg)
            case "SET_CITY":
